...
// Expanded responses with mystical predictions
let responses = [
    ...
    "the stars align to reveal great fortune in your endeavors",
...
// Array to store active scroll elements
let activeScrolls = [];
...
circleContainer.addEventListener("mousedown", (e) => {
    ...
    // Show the rubbing progress
    if (clickCount <= maxClicks) {
        createFloatingText(e.clientX, e.clientY, `${clickCount}/${maxClicks}`);
    }

    // If this is the third click, start the timer
    if (clickCount >= maxClicks) {
        ...
        timer = setTimeout(() => {
            ...
            // Create scrolls with predictions
            createScrolls();
...
// Create floating text to show rubbing progress
function createFloatingText(x, y, text) {
    ...
    floatingText.style.color = '#98A886';
    ...
    setTimeout(() => {
        floatingText.style.transition = 'opacity 0.5s ease-out, transform 0.5s ease-out';
